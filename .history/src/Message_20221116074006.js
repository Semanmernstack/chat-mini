...
function Message({ timestamp, user, message }) {
  return (
    <div className=" flex items-center mx-2 my-3 border    p-1 lg:p-3  rounded-b-xl w-70 lg:w-full  ">
      ...
      <h3 className="m-x-3 ml-3  font-serif flex items-center font-bold text-xs lg:xl">{user?.displayName}<span className="text-xs ml-2">{new Date(timestamp?.toDate()).toUTCString()}</span>  </h3> 
